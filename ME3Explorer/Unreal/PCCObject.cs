...
namespace ME3Explorer.Unreal
{
    public class PCCObject
    {
        ...
        private uint version { get { return BitConverter.ToUInt32(header, 4); } }
            ...
            public string ClassName
            {
                get
                {
                    ...
                    if (val < 0)
                    {
                        ...
                        {
                            return pccRef.Names[pccRef.Imports[val * -1 - 1].idxObjectName];
                        }
                    }
                    else if (val > 0)
                    {
                        ...
                        {
                            return pccRef.Names[pccRef.Exports[val].idxObjectName];
                        }
                    }
                    else
                        return "Class";
        ...
        public PCCObject(string pccFilePath, bool fullFileInMemory = false)
        {
            ...
            using (FileStream pccStream = File.OpenRead(pccFileName))
            {
                ...
                Debug.WriteLine("Magic: 0x" + magic.ToString("X8"));
                // BitConverter isn't working?!?!
                if (magic == 0x9E2A83C1)
                {
                    BitConverter.IsLittleEndian = true;
                    Debug.WriteLine("block 1");
                    IsLittleEndian = true;
                }
                else
                {
                    Debug.WriteLine("block 2");
                ...
                for (int i = 0; i < NameCount; i++)
                {
                    ...
                    strLength = ForceSwapEndianness(strLength);
                    Debug.WriteLine("String length: " + strLength * -2);
                    string name = listsStream.ReadString(strLength * -2, true, Encoding.Unicode);
                    Names.Add(name);
                ...
                for (int i = 0; i < ImportCount; i++)
                {
                    ...
                    Debug.WriteLine("Read " + i + ": " + imp.ObjectName);
                    Imports.Add(imp);
                }
                ...
                for (int i = 0; i < ExportCount; i++)
                {
                    ...
                    count = ForceSwapEndianness(count);
