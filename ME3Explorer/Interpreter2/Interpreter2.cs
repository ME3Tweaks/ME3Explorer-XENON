...
namespace ME3Explorer.Interpreter2
{
    public partial class Interpreter2 : Form
    {
        ...
        public void InitInterpreter(TalkFile editorTalkFile = null)
        {
            if(editorTalkFile != null)
            {
                InitInterpreter(new TalkFiles(editorTalkFile) );
            } else
            {
                InitInterpreter((TalkFiles) null);
            }
        }
        ...
        public TreeNode GenerateTree(TreeNode input, List<PropHeader> headersList)
        {
            ...
            foreach (PropHeader header in headersList)
            {
                ...
                else
                {
                    ...
                    {
                        if (pcc.getNameEntry(header.name) == "RawData")
                        {
                            TreeNode suppressed = new TreeNode("Suppressed "+header.size+" bytes of a RAWDATA array.");
                            ret.Nodes.Add(suppressed);
                            continue;
                        }
