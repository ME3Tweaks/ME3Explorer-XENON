    ...
    public partial class PCCEditor2 : Form
    {
        ...
        public void DumpBin()
        {
            ...
            if (d.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                byte[] buff = pcc.Exports[n].Data;
                int start = prop[prop.Count - 1].offend;
        ...
        private void dumpGUIFilesToolStripMenuItem_Click(object sender, EventArgs e)
        {
            foreach (PCCObject.ExportEntry exp in pcc.Exports)
            {
                if (exp.ClassName == "GFxMovieInfo")
                {
                    List<ME3Explorer.Unreal.PropertyReader.Property> prop = ME3Explorer.Unreal.PropertyReader.getPropList(pcc, exp.Data);
                    string filename = exp.PackageName + "." + exp.ObjectName + ".swf";
